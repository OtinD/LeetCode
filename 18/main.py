...
class N_Sum(object):
    ...
    def four_Sum(self, mlist, target, Sorted=False, OO=False):
        if not Sorted:
            mlist.sort()
        i = 0
        ans_list = []
        while i < len(mlist) and 4 * mlist[i] <=target:
            new_target = target - mlist[i]
            pre_ans_list = self.three_Sum(mlist[i + 1: len(mlist)], new_target, Sorted=True, OO=OO)
            while pre_ans_list != []:
                register = pre_ans_list.pop()
                ans_list.append([mlist[i], register[0], register[1], register[2]])
            if OO and ans_list != []:
                break
            i += 1
            while i < len(mlist) and mlist[i - 1] == mlist[i]:
...
class Solution(object):
    def fourSum(self, nums, target):
        ...
        :rtype: List[List[int]]
        """
        return N_Sum().four_Sum(nums, target)
